...
class StreamCapture:
    ...
    def connect(self) -> bool:
        ...
        try:
            ...
            # FFmpeg 命令参数（最大兼容性 + 低延迟）
            cmd = [
                'ffmpeg',
                '-rtsp_transport', 'tcp',
                '-buffer_size', '1048576',  # 1MB 缓冲
                ...
                # 不使用 -vf fps 过滤器，让 FFmpeg 输出原始帧，由 Python 层控制采样
            ...
            if self.process.poll() is not None:
                ...
                print(f"❌ FFmpeg 进程启动失败")
                print(f"   返回码：{self.process.returncode}")
                if stderr_data:
            ...
            if ret and frame is not None:
                ...
                if self.process.poll() is not None:
                    ...
                    print(f"   FFmpeg 进程已退出，返回码：{self.process.returncode}")
                    print(f"   错误信息：{stderr_output[:300]}")
                return False

        except FileNotFoundError as e:
            print(f"❌ 找不到 FFmpeg 程序！")
            print(f"   请确保已安装 FFmpeg 并添加到 PATH")
            print(f"   下载地址：https://www.gyan.dev/ffmpeg/builds/")
            return False
    ...
    def _read_frame(self):
        ...
        try:
            ...
            print(f"⚠️ 读取帧异常：{type(e).__name__}: {e}")
            import traceback
            traceback.print_exc()
            return False, None
        ...
        try:
            ...
            while not soi_found:
                if time.time() - start_time > timeout:
                    print(f"\r⚠️ 等待 SOI 标记超时", end='', flush=True)
                    return False, None
                
                search_count += 1
                if search_count > max_search_iterations:
                    print(f"\r⚠️ SOI 搜索次数过多，重置缓冲区", end='', flush=True)
                    search_buffer.clear()
                    search_count = 0
                ...
                if not chunk or len(chunk) == 0:
                    if self.process.poll() is not None:
                        print(f"\r⚠️ FFmpeg 进程已退出", end='', flush=True)
                        return False, None
                    time.sleep(0.01)
                    continue
            ...
            while not eoi_found:
                if time.time() - start_time > timeout:
                    print(f"\r⚠️ 读取 JPEG 数据超时", end='', flush=True)
                    return False, None
                
                read_count += 1
                if read_count > max_read_iterations:
                    print(f"\r⚠️ EOI 读取次数过多，放弃当前帧", end='', flush=True)
                    return False, None
                
                if len(buffer) > max_jpeg_size:
                    print(f"\r⚠️ JPEG 数据过大 ({len(buffer)//1024}KB)，放弃", end='', flush=True)
                    return False, None
            ...
            if len(jpeg_data) < 100:
                print(f"\r⚠️ JPEG 数据过小 ({len(jpeg_data)} 字节)", end='', flush=True)
                return False, None
            ...
            if frame is None or frame.size == 0:
                print(f"\r⚠️ JPEG 解码失败", end='', flush=True)
                return False, None
            
            return True, frame
            
        except Exception as e:
            print(f"⚠️ JPEG 帧读取异常：{type(e).__name__}: {e}")
            import traceback
            traceback.print_exc()
            return False, None
